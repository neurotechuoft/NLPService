...
def load_words(check_grammar=True):
    with codecs.open(resources_path + "w2.txt", "r", encoding='utf-8', errors='ignore') as fdata:
    ...
    grams.to_pickle(resources_path + "words.pkl")
    return grams
...
def check_cache(start_word):
    ...
    """
    try:
        pop_trie = open(resources_path + 'popular_trie.pkl', 'rb')
        pop_dict = open(resources_path + 'dict.pkl', 'rb')
    ...
    try:
        long = open(resources_path + 'trie.pkl', 'rb')
    except IOError:
        load_data()
        long = open(resources_path + 'trie.pkl', 'rb')
...
def perform_setup(check_time, fix_grammar):
    start = time.time()
    load_words(check_grammar=fix_grammar)
    load_data()
    popular_trie()
    if check_time:
        # Last test time on old laptop: 26 seconds
        print("Seconds to load everything: ", time.time() - start)
